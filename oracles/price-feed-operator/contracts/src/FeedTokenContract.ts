 ...
 */
class Erc20Contract extends SmartContract implements Erc20 {
  // constant supply
  SUPPLY = UInt64.from(10n ** 18n);

  deploy(args: DeployArgs) {
    super.deploy(args);
    this.account.tokenSymbol.set('FCN');
  ...
  // ERC20 API
  name(): CircuitString {
    return CircuitString.fromString('FeedCoin');
  }
  symbol(): CircuitString {
    return CircuitString.fromString('FCN');
  }
  decimals(): Field {
    return Field(9);
  }
...
export class FeedCoinContract extends Erc20Contract {

...
const paths = require('env-paths')('zkoracle-docs-preview', { suffix: '' });;
...
function makePlaybook(docsDir, componentDirs) {
  ...
  const playbook = yaml.safeLoad(fs.readFileSync(path.join(docsDir, 'scripts/docs.zkoracle.bbb/playbook.yml')));
...
function build(docsDir, playbook) {
  proc.spawnSync('pnpm', ['run', 'build:custom', playbook], {
    cwd: docsDir +"/scripts/docs.zkoracle.bbb",
    stdio: 'inherit',
  });
}
...
function setupDocsDir(docsDir) {
  ...
  if (fs.existsSync(docsDir)) {
    ...
    proc.spawnSync('git', [
      ...
      '--branch',
      'mina-nav-jan-24',
      'https://github.com/ubinix-warun/zkOracle-OCO.git',
      '--depth=1',
      docsDir,
    ], {
      stdio: 'inherit',
...
function updateVersion(componentDir) {
  ...
  // npm and pnpm seem to use two different variable names.
